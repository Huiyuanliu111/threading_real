...
class LocalPolicy(torch.nn.Module):
 ...
 def __init__(self,client):
  ...
  self.metadata=m;self.task=m['task'];print('[openpi] local model:',m,flush=True)
 def predict_action(self,obs):
  payload={'state':obs['agent_pos'][0,-1].cpu().numpy(),'prompt':self.task}
  for name,key in [('front','frontview'),('side','sideview')]:
...
class LocalBackend:
 ...
 def get_server_metadata(self):
  return dict(kind='threading_openpi_tcp6',horizon=50,state_dim=9,action_dim=6,fps=30,checkpoint=self.checkpoint,task='insert the grasped block through the needle')
...
def main():
 ...
 parser.set_defaults(gripper_force=70.0,prepare_initial_grasp=True,device='cpu',policy_hz=30,stream_hz=480,execute_steps=1,image_size=224,synchronous=True,sync_timeout=5.0,weights='model')
 ...
 policy=LocalPolicy(LocalBackend(args.checkpoint,args.run_config))
 def observe(side,wrist,front,q,width,image_size,pre_resize_image_size=None,tcp_position=None):
  ...
  images=[torch.from_numpy(letterbox(im).transpose(2,0,1).copy()) for im in (side,front)]
  return ObservationFrame(sideview=images[0],wrist=None,frontview=images[1],agent_pos=torch.from_numpy(state),timestamp=time.monotonic(),tcp_pos=torch.from_numpy(pos[0].astype(np.float32)))
 ...
 finally:cartesian.load_deployment_policy,cartesian.make_observation,cartesian.GripperController=original
