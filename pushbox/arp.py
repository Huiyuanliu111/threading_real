...
class GMMPredictor(TokenPredictorInterface):
    ...
    def forward(self, embs, log_prob=False, split_distributions=False, **extra_contexts) -> Union[Dict[str, List[Tensor]], D.Distribution]:
        ...
        if self.num_latents > 1:
            ...
            scales = torch.exp(0.5 * raw_scales).clamp(min=1e-3)
            if not self.training and self.low_var_eval:
                scales[:] = (1e-5 * means).abs().clamp(min=1e-6)
...
@register_token_predictor("regression")
class RegressionPredictor(TokenPredictorInterface):
    """Deterministic continuous-token predictor with a robust regression loss."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(n_embd, token, **kwargs)
        if loss not in {"smooth_l1", "l1", "mse"}:
            raise ValueError(f"Unsupported regression loss {loss!r}")
        if beta <= 0:
            raise ValueError(f"Smooth-L1 beta must be positive, got {beta}")
        self.loss = loss
        self.beta = float(beta)
        self.label_name = label_name
        self.mlp = Mlp(
            in_features=n_embd,
            hidden_features=n_embd,
            out_features=token["dim"],
        )
    ...
    def forward(
        ...
        embs,
        log_prob=False,
        split_distributions=False,
        **extra_contexts,
    ):
        prediction = self.mlp(embs)
        if not self.training:
            return prediction

        label = extra_contexts[self.label_name]
        if self.loss == "smooth_l1":
            loss = F.smooth_l1_loss(prediction, label, beta=self.beta)
        elif self.loss == "l1":
            loss = F.l1_loss(prediction, label)
        else:
            loss = F.mse_loss(prediction, label)
        result = {f"{self.loss}_loss": [loss]}
        if log_prob:
            # Regression heads do not define a normalized likelihood. This
            # score is only used as a per-token diagnostic when requested.
            result["log_prob"] = [
                -F.smooth_l1_loss(
                    prediction,
                    label,
    ...
    def sample(
        self,
        predicts_of_curr_regs: List[Union[Tensor, D.Distribution]],
        do_sample: bool,
        **extra_contexts,
    ) -> Tensor:
        del do_sample, extra_contexts
        result = []
        for prediction in predicts_of_curr_regs:
            if not isinstance(prediction, Tensor):
                raise TypeError(
                    "RegressionPredictor expects tensor predictions, "
