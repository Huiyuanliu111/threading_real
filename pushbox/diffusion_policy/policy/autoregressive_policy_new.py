...
import pushbox.diffusion_policy.model.vision.crop_randomizer as dmvc
...
class ARPolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        self.inference_chunk_size = None  # set externally to enable variable-chunk inference (Method 3)
        #endregion ################################################################
    ...
    def predict_action(self, batch_or_obs_dict, training=False) -> Union[Dict[str, torch.Tensor], torch.Tensor]:
        ...
        # Method 3: variable chunk size at inference time (no retraining needed).
        # inference_chunk_size=h generates min(h+1, max_chunk_size) action tokens and executes h of them.
        if not training and self.inference_chunk_size is not None:
            _h = self.inference_chunk_size
            _inference_horizon = min(_h + 1, self.policy.cfg.max_chunk_size)
            _n_action_steps = _inference_horizon - 1
        else:
            _inference_horizon = self.horizon
            _n_action_steps = self.n_action_steps
