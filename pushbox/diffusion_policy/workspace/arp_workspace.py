...
from pushbox.diffusion_policy.policy.autoregressive_policy_new import ARPolicy
...
class ARPWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: ARPolicy = hydra.utils.instantiate(cfg.policy)

        self.ema_model: ARPolicy = None
    ...
    def run(self):
        ...
        # self.model.set_normalizer(normalizer) # NOTE
        # if cfg.training.use_ema:
        ...
        add_full_horizon_eval = getattr(cfg.training, 'add_full_horizon_eval', False)

        if add_full_horizon_eval:
            _env_runner_cfg = deepcopy(cfg.task.env_runner)
            _env_runner_cfg.n_action_steps = cfg.policy.horizon - 1
            env_runner_full_horizon = hydra.utils.instantiate(_env_runner_cfg, output_dir=self.output_dir)
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        # compute loss
                        raw_loss_dict = self.model.compute_loss(batch) # NOTE
                        raw_loss = sum(raw_loss_dict.values())
                        loss = raw_loss / cfg.training.gradient_accumulate_every
                        loss.backward()

                        # measure gradient norm
                        step_log['grad_norm'] = compute_grad_norm(self.model).item()
                ...
                if (self.epoch % cfg.training.rollout_every) == 0:
                    ...
                    for k, v in runner_log.items():
                        if 'mean_score' in k and 'test' in k:
                            tqdm.tqdm.write(f"[Epoch {self.epoch}] {k}: {v}")

                    if add_full_horizon_eval:
                        old_n_action_steps = policy.n_action_steps
                        policy.n_action_steps = cfg.policy.horizon - 1
                        _log = env_runner_full_horizon.run(policy)
                        policy.n_action_steps = old_n_action_steps
                        for k, v in _log.items():
                            if 'mean_score' in k and 'test' in k:
                                tqdm.tqdm.write(f"[Epoch {self.epoch} (full horizon)] {k}: {v}")
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    with torch.no_grad():
                        ...
                        with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}", 
                            ...
                            for batch_idx, batch in enumerate(tepoch):
                                batch = dict_apply(batch, lambda x: x.to(device, non_blocking=True))
                                val_loss_dict = self.model.compute_loss(batch)
                                for k, v in val_loss_dict.items():
                                    val_losses['val.' + k].append(v.item())
                                val_losses['val_loss'].append(sum(val_loss_dict.values()))
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        result = policy.predict_action(obs_dict) # NOTE
                ...
                if self.epoch == getattr(cfg.training, 'stop_at_epoch', 5000):
