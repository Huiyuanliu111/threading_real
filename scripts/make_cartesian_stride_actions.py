...
def cumulative_actions(actions: np.ndarray, episode_ids: np.ndarray, stride: int) -> np.ndarray:
    ...
    for episode in np.unique(episode_ids):
        ...
        for t in range(len(rows)):
            ...
            out[rows[t], :3] = window[:, :3].sum(axis=0)
            rotation = np.eye(3)
            for delta in window:
...
def convert(source: Path, output: Path, stride: int) -> dict:
    ...
    try:
        ...
        for path in sorted((building / "data").rglob("*.parquet")):
            ...
            result = cumulative_actions(actions, episodes, stride)
        ...
        report = {"source": str(source), "output": str(output), "stride_frames": stride, "source_fps": float(info["fps"]), "recommended_policy_hz": float(info["fps"]) / stride, "total_frames": int(len(actions)), "max_translation_m": float(np.linalg.norm(actions[:, :3], axis=1).max())}
...
def main() -> int:
    p = argparse.ArgumentParser(description=__doc__); p.add_argument("source", type=Path); p.add_argument("output", type=Path); p.add_argument("--stride", type=int, default=5)
    args = p.parse_args()
    print(json.dumps(convert(args.source, args.output, args.stride), indent=2)); return 0
