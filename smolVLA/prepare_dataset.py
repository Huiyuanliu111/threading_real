#!/usr/bin/env python3
"""Build the 6 Hz, two-camera LeRobot dataset used by SmolVLA.

The input actions are already Cartesian deltas from frame t to t+5. The source
dataset is stored at 30 Hz, so one frame out of every five is retained. This
makes adjacent actions in a SmolVLA action chunk non-overlapping.
"""
...
def convert(source: Path, output: Path, repo_id: str, stride: int, overwrite: bool) -> dict:
    ...
    expected_actions = ["dx", "dy", "dz", "drotvec_x", "drotvec_y", "drotvec_z", "dgripper"]
    ...
    report_path = output / "meta" / "smolvla_preparation_report.json"
